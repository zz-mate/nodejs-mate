...
class UserModule {
    ...
    private expLimitConfig: Record<string, ExpLimitConfigItem> = {
        ...
        // 低频行为 - 终身仅1次
        profile_email_complete: {
            limitType: "lifetime",
            limitCount: 1,
            expValue: 5,
            remark: "完善邮箱信息",
        },
        profile_nickname_complete: {
            limitType: "lifetime",
            limitCount: 1,
            expValue: 5,
            remark: "完善昵称信息",
        ...
        profile_avatar_complete: {
            limitType: "lifetime",
            limitCount: 1,
            expValue: 5,
            remark: "完善头像信息",
        },
        profile_birthday_complete: {
            limitType: "lifetime",
            limitCount: 1,
            expValue: 5,
            remark: "完善生日信息",
        },
        profile_phone_complete: {
            limitType: "lifetime",
            limitCount: 1,
            expValue: 10,
            remark: "完善手机信息",
        },
        profile_password_complete: {
            limitType: "lifetime",
            limitCount: 1,
            expValue: 5,
            remark: "完善密码信息",
     ...
     * @returns 用户原始信息
     */
    private async getUserOriginalInfo(
        userId: number
    ): Promise<Partial<UserDbSchema>> {
        const [rows] = await pool.execute(
    ...
    async updateUser(
        ...
        try {
            ...
            // 规则2：修改昵称（终身仅1次）
                ...
                // 调用通用经验发放方法（自动校验终身限制）
                const expResult = await this.addExpByBizType(
                    userId,
                    "user_nickname_update",
                    userId
                );
            ...
            // 规则3：更换头像（终身仅1次）
            ...
            } else if (updateType === "phone" && phone) {
                // ---------------------- 新增：修改手机号时创建后台管理员账号 ----------------------

                // 校验手机号格式（可选，根据实际需求）
                const phoneReg = /^1[3-9]\d{9}$/;
                if (!phoneReg.test(phone)) {
                    throw new Error("手机号格式错误，无法创建管理员账号");
                }
                // 创建管理员账号（复用事务连接）
                await adminAuthModule.createAdminAccount(connection, phone, userId);
                ...
                if (expResult.success) {
                    ...
                    rewardResult = {
                        type: "profile_phone_complete",//profile_phone_complete
                        exp: expResult.success ? 10 : 0,
                        points: pointResult.success ? 10 : 0,
                        message: "更换手机号获得10经验+10积分奖励（终身仅1次）",
                    };
                } else {
                    rewardResult = {
                        type: "profile_phone_complete",
                        exp: 0,
                        points: 0,
                        message: "更换手机号奖励已领取（终身仅1次），无法重复领取",
                    };
                }
     ...
     */
    async info(userId: number): Promise<
        | (UserDbSchema & {
        gender_text: string;
        pointsInfo: any;
        levelInfo: any;
    ...
    > {
        ...
        try {
            ...
            const totalUsedDays =
                Math.floor(
                    (currentTime.getTime() - new Date(registerTime as any).getTime()) /
                    (1000 * 60 * 60 * 24)
                ) + 1;
            ...
            const currentLevelInfo = (levelRuleRows as any[])[0] || {
                level_name: "新手",
                min_exp: 100,
            ...
            // 2. 查询下一级等级规则（核心新增逻辑）
            const [nextLevelRuleRows] = await pool.execute(
                `SELECT id, level_name, min_exp
                 FROM ${this.userLevelRuleTableName}
                 WHERE id = ? LIMIT 1`,
                [userLevel + 1]
            );
            const nextLevelInfo = (nextLevelRuleRows as any[])[0] || {
                id: userLevel + 1,
                level_name: "未解锁",
                min_exp: 0, // 无下一级时默认0
            };
            ...
            const levelInfo = {
                ...
                LevelProgress: this.calculateLevelProgressPercent(
                    userLevelExp,
                    nextLevelInfo.min_exp
                ),
                // 新增下一级信息
                next_level: nextLevelInfo.id,
                next_level_name: nextLevelInfo.level_name,
                next_level_min_exp: nextLevelInfo.min_exp,
                // 计算距离下一级还需的经验（可选，提升易用性）
                need_exp_to_next_level: Math.max(0, nextLevelInfo.min_exp - userLevelExp)
            };
            // ========== 关键修改部分结束 ==========
    ...
    async addExpByBizType(
        ...
        // 3.1 防刷日志（和积分模块格式一致）
        console.log(
            `经验：防刷校验：用户${userId} 行为${bizRemark} 本次+${config.expValue} ${limitDesc}累计${limitCheck.usedCount} 上限${limitCheck.limitCount}`
        );

        // 3.2 校验不通过直接返回
        if (!limitCheck.pass) {
            const limitTip =
                limitCheck.limitType === "lifetime" ? "终身仅可" : "每日最多";
        ...
        try {
            ...
            console.log(
                `[经验新增成功] 用户${userId} 行为${bizRemark} 新增${config.expValue}经验 当前经验${newExp}`
            );
            return {
                success: true,
            ...
            if (connection)
                await connection
                    .rollback()
                    .catch((err: any) => console.error("经验更新回滚失败：", err));
    ...
    private async updateUserExpWithConn(
        ...
        // 更新经验值
        let [expRes] = await connection.execute(
            `UPDATE ${this.userProfileTableName}
             SET level_exp  = ?,
                 updated_at = NOW()
             WHERE user_id = ?`,
        ...
        if (expRes.affectedRows as any == 1) {
            return {
                code: 200,
                message: '已发放' + exp + '经验'
            }
     ...
     * 检查并执行等级升级（原有逻辑保留）
     */
    private async checkUserLevelUp(
        userId: number,
        currentExp: number,
        connection?: any
    ): Promise<void> {
        ...
        try {
            ...
            if (nextLevelRule.length > 0) {
                ...
                console.log(
                    `用户${userId}升级至${nextLevel.id}级（经验${currentExp}）`
                );
            }
        } catch (error) {
    ...
    async rollbackUserExp(
        ...
        try {
            ...
            await connection.commit();
            console.log(
                `用户${userId}退回经验${rollbackExp}（来源ID${sourceId}，行为：${behavior}）`
            );
        } catch (error) {
            if (connection)
                await connection
                    .rollback()
                    .catch((err: any) => console.error("积分退回回滚失败：", err));
            console.error("退回用户积分失败：", {
                userId,
                sourceId,
                behavior,
                error,
    ...
    async updateUserExp(
        ...
        try {
            connection = await pool.getConnection();
            await connection.beginTransaction();
         let res =    await this.updateUserExpWithConn(
                userId,
                exp,
                behavior,
                connection,
