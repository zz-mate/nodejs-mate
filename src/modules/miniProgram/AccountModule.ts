      ...
      (allDataResult as any[]).forEach((item) => {
        ...
        if (item.account_id) {
          ...
          const account: AccountDbSchema = {
            ...
            parent_account_id: item.account_parent_id
              ? Number(item.account_parent_id)
              : null,
            account_id: item.account_super_id
              ? Number(item.account_super_id)
              : null,
          ...
          categoryMap[categoryId].category_remaining_amount = formatAmount(
            Number(categoryMap[categoryId].category_remaining_amount) +
