...
import HttpStatus from "../../utils/HttpError.ts";
// @ts-ignore
...
class AuthModule {
    ...
    async createUser(phone: string): Promise<string> {
        ...
        const defaultData = {
            ...
            role: "user", // 默认角色
            last_login_at: new Date(), // 转MySQL日期格式
            created_at: new Date(), // 可选：数据库自动生成，也可手动传
            updated_at: new Date(), // 可选：数据库自动更新
        ...
        try {
            ...
            const newUser: any = {
                ...defaultData,
                id: (result as any).insertId, // 注意是 user_id（下划线）
            };
            ...
            const defaultBookData = {
                    uuid: uuidv4(), // 36位唯一UUID（表中 uk_uuid 约束）
                    user_id: (result as any).insertId, // 手机号（可选，表中 uk_phone 约束）
            ...
            const [bookResult] = await pool.execute(
                `INSERT INTO ${this.bookTableName}
                 (uuid, user_id, name, type, currency, description, is_default, is_active, created_at, updated_at,
                  deleted_at)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [
            ...
            await pool.execute(
                `UPDATE ${this.userTableName}
                 SET default_book_id = ?
                 WHERE id = ?`, // 关键：限定更新当前用户
                [(bookResult as any).insertId, (result as any).insertId]
            );
            safeUser.default_book_id = (bookResult as any).insertId
            ...
            if (err.code === "ER_NO_REFERENCED_ROW_2") {
                ...
                throw new HttpError(`唯一键冲突：username/phone/uuid/email已存在`, 403);
            } else {
