...
class PointModule {
  ...
  async addPoints(
    ...
    if (points <= 0) {
      return {
        success: false,
        message: "新增积分需为正数，扣减积分请调用 deductPoints 方法",
      };
    }
    ...
    console.log(
      `积分：防刷校验：用户${userId} 行为${bizRemark} 本次+1 ${limitDesc}累计${limitCheck.usedCount} 上限${limitCheck.limitCount}`
    );
    ...
    // 4. 防重复发放（传了bizId才校验）
    if (bizId) {
      const isDuplicate = await this.checkDuplicatePoints(
        userId,
        bizType,
        bizId
      );
    ...
    if (this.isProfileLifetimeBiz(bizType)) {
      const isProfileCompleted = await this.checkProfileCompleted(
        userId,
        bizType
      );
    ...
    try {
      ...
      console.log(
        `[积分新增成功] 用户${userId} 新增${points}积分，总积分：${totalPoints}，可用积分：${availablePoints}`
      );
      return {
        success: true,
  ...
  async deductPoints(
    ...
    if (bizId) {
      ...
      if (isDuplicate) {
        return {
          success: false,
          message: `该业务(${bizType}-${bizId})已扣减过积分，请勿重复操作`,
        };
      }
    }

    // 4. 校验可用积分是否足够
    const { totalPoints: currentTotal, availablePoints: currentAvailable } =
      await this.getUserAllPoints(userId);
    ...
    try {
      ...
      await connection.commit();
      console.log(
        `[积分扣减成功] 用户${userId} 扣减${points}积分，总积分：${totalPoints}，可用积分：${availablePoints}`
      );
      return {
        success: true,
  ...
  private isProfileLifetimeBiz(bizType: string): boolean {
    return (
      bizType.startsWith("profile_") &&
      this.pointLimitConfig[bizType]?.limitType === "lifetime"
    );
  }
   ...
   */
  private async checkProfileCompleted(
    userId: number,
    bizType: string
  ): Promise<boolean> {
    let checkSql = "";
  ...
  async getActionLimitStatus(
    ...
    const remainCount = Math.max(
      0,
      limitCheck.limitCount - limitCheck.usedCount
    );
    return {
      usedCount: limitCheck.usedCount,
      remainCount,
