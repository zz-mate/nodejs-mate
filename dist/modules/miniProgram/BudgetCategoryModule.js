...
class BudgetCategoryModule {
    ...
    async create(params) {
        ...
        try {
            ...
            // 3.根据日期查询 获取已经消费的金额
            let actual_amount = await this.getActualAmount(defaultData.user_id, defaultData.book_id, defaultData.budget_id, defaultData.category_id);
     ...
     */
    async getActualAmount(user_id, book_id, budget_id, category_id) {
        // 1. 参数校验（避免无效查询）
        if (!user_id || !book_id || !budget_id || !category_id) {
            throw new Error('参数错误：user_id、book_id、budget_id、category_id 均为必填项');
        }
        try {
            ...
            const [rows] = await pool.execute(`SELECT IFNULL(SUM(amount), 0) AS total_expense
                 ...
                 WHERE user_id = ?
                   ...
                   AND category_id = ?
                   AND type = 2 -- 仅计算支出类
                `, // 假设账单表的日期字段为 bill_date，可根据实际字段名调整
            [user_id, book_id, budget_id, category_id]);
            // 4. 处理结果（转为数字并保留2位小数）
            // @ts-ignore
     ...
     */
    async findById(user_id, book_id, budget_id, category_id) {
        // 1. 严格参数校验
        // 校验ID参数
        ...
        try {
            ...
            const [rows] = await pool.execute(`SELECT 1
                 ...
                 WHERE user_id = ?
                   AND book_id = ?
                   AND budget_id = ?
                   AND category_id = ? LIMIT 1`, [user_id, book_id, budget_id, category_id]);
